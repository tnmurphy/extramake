...
GMK_EXPORT char *
func_reverse (const char *func_name, unsigned int argc, char **argv)
{
    char *result = NULL;
    ...
    /* Work out how long the result string has to be which may be less than the length if
     * the input has multiple spaces in it since the output won't have multiple spaces.
     */
    result_index = 0;
    char *c = startptr;
    int inspace = 1;
	...
	if (*c == ' ' || *c == '\t') {
	    if (inspace == 0) {
                result_index++; /* transition from word to space - will need a separator in the result */
	    }
	    inspace = 1;
	} else {
    ...
    /* fill the result string starting from the end coming back towards the beginning
     * placing one token at a time from the source. A single space is inserted
     * between each token. 
     */
    while (*startptr != '\0') {
...
GMK_EXPORT
int
reverse_gmk_setup (const gmk_floc *flocp)
{
    NOTUSED(flocp);
