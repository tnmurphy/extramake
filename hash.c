...
/*
 * siphash24 - returns a 16 character hexadecimal hash value of the input
 * string. An optional key value may be supplied as a second parameter and it
 * must be 16 characters.
 *
 * On error this function returns an empty string.
 */
unsigned char default_key[] =
    "\x4c\xe4\x00\x24\x4c\x97\x14\xe4\x1d\x96\x2c\xce\x9f\x48\x6f\x2d";
GMK_EXPORT char *func_siphash24(const char *func_name, unsigned int argc,
                                char **argv) {
  ...
  (void)snprintf(the_hash_string, HASH_STRING_SIZE + 1,
                 "%02x%02x%02x%02x%02x%02x%02x%02x", the_hash[0], the_hash[1],
                 the_hash[2], the_hash[3], the_hash[4], the_hash[5],
                 the_hash[6], the_hash[7]);
...
GMK_EXPORT
int hash_gmk_setup(const gmk_floc *flocp) {
  NOTUSED(flocp);
